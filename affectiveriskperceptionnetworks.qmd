...
packages <- c("tidyverse", "naniar", "knitr", "ggstatsplot", "bootnet", "ppcor", "smacof", "qgraph", "NetworkComparisonTest", "mgm")
...
**Add text on Mixed Gaussian Models with moderation analysis!**
...
We then retrieved a robust model of the graph using the `bootnet` package [@epskamp2018b]. The model uses the `EBICglasso` algorithm, which searches for the model that locally minimises the extended Bayesian Information Criterion [@epskamp2018b; @foygel2010]. The estimation is done using Spearman correlations because our data are expected non-normally distributed. While polychoric correlations would be possible, the small sample sizes in sub-population comparison may result in a non-positive definite correlation matrix, which disables analysis.[^2] We retrieved both non-parametric and case-dropping networks for analysis.

[^2]: To test equivalency, we retrieved a bootstrapped network using polychoric correlations with the `cor_auto` method in `bootnet`, and correlated the resulting network with the network retrieved using Spearman correlations. The resulting Pearson correlation was 0.992. Thus, under our conditions, the two methods produce virtually the same results, and we opt for the simpler Spearman correlations due to calculation efficiency.
...
# WIP: Mixed Gaussian model with mediation analysis

We then estimated a moderated Mixed Gaussian Model (MGM) following Haslbeck's [-@haslbeck2022] mediation analysis framework. We created a four-level categorical variable representing different modes of experience of fire: either an individual has not experienced a fire (0), has experienced a near-miss but not a direct fire (1), has experienced a direct fire but not a near-miss (2), or has experienced both a direct fire and a near-miss situation (3).

We then fitted an MGM using 10-fold cross-validation and the OR rule for regression estimate combination.

\[TESTING RIGHT NOW WITH TWO CONTROL VARIABLES INSTEAD OF ONE, SEPARATELY\].

```{r}
#| label: mgm-estimation
#| fig-cap: "Mixed Gaussian graphical models with moderation analysis, conditioned by categories."
#| sub-cap:
#| -  "No direct experience"
#| -  "Direct experience"
#| -  "No near-miss experinece"
#| -  "Near-miss experience"
#| layout: [[1,1],[1,1]]
#| message: false
#| results: false
...
qgraph(
  input = mgm_cond_1[[1]]$pairwise$wadj,
  edge.color = mgm_cond_1[[1]]$pairwise$edgecolor,
  layout = "circle",
  maximum = 0.5,
  edge.labels = TRUE
)
...
qgraph(
  input = mgm_cond_1[[2]]$pairwise$wadj,
  edge.color = mgm_cond_1[[2]]$pairwise$edgecolor,
  layout = "circle",
  maximum = 0.5,
  edge.labels = TRUE
)
...
qgraph(
  input = mgm_cond_2[[1]]$pairwise$wadj,
  edge.color = mgm_cond_2[[1]]$pairwise$edgecolor,
  layout = "circle",
  maximum = 0.5,
  edge.labels = TRUE
)
...
qgraph(
  input = mgm_cond_2[[2]]$pairwise$wadj,
  edge.color = mgm_cond_2[[2]]$pairwise$edgecolor,
  layout = "circle",
  maximum = 0.5,
  edge.labels = TRUE
)
